...
% Last Modified by GUIDE v2.5 20-May-2018 22:28:58
...
if ~isempty(handles.Result)
    axes(handles.axes1); imshow(handles.Result.Image); title('原图像');
    axes(handles.axes2); imshow(handles.Result.hist); title('灰度变换图像');
end
...
if ~isempty(handles.Result)
    axes(handles.axes1); imshow(handles.Result.Image); title('原图像');
    axes(handles.axes2); imshow(handles.Result.Medfilt); title('中值滤波去噪图像');
end
...
if ~isempty(handles.Result)
    axes(handles.axes1); imshow(handles.Result.Image); title('原图像');
    axes(handles.axes2); imshow(handles.Result.Bw); title('二值化处理图像');
end
...
if ~isempty(handles.Result)
    axes(handles.axes1); imshow(handles.Result.Image); title('原图像');
    axes(handles.axes2); imshow(handles.Result.Enance); title('对比度增强图像');
end
...
[filename, pathname] = uiputfile({'*.jpg;*.tif;*.png;*.gif','All Image Files';...
    ...
    fullfile(pwd, 'Result/result.jpg'));
...
% --- Executes on button press in pushbuttoSaveResult.
...
try
    if ~isempty(handles.File)
        raw = [];
        foldername = fullfile(pwd,'Result');
        ...
        F{2, 6} = handles.Result.BwWidthMin;
